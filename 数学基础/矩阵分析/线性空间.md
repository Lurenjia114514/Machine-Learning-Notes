...
设 $V_1$ 和 $V_2$ 为线性空间 $V$ 的子空间, 两者的交空间为
$$
V_1\cap V_2=\{\boldsymbol{x}\in V:\boldsymbol{x}\in V_1,\boldsymbol{x}\in V_2\}
$$

两者的和空间为
$$
V_1+ V_2=\{\boldsymbol{x}_1+\boldsymbol{x}_2:\boldsymbol{x}_1\in V_1,\boldsymbol{x}_2\in V_2\}
$$

且有维数公式
$$
\mathrm{dim}V_1+\mathrm{dim}V_2=\mathrm{dim}(V_1\cap V_2)+\mathrm{dim}(V_1+ V_2)
$$

当和空间 $V_1+ V_2$ 中任意向量只能唯一表示为两线性空间中向量之和, 则称 $V_1+ V_2$ 为直和, 记为 $V_1\oplus V_2$, 称 $V_1$ 和 $V_2$ 为互补子空间, 且有维数公式
$$
\mathrm{dim}(V_1\oplus V_2)=\mathrm{dim}V_1+\mathrm{dim}V_2
$$
...
对于子空间中任意向量 $\boldsymbol{x}\in\mathcal{S}_0$, 可以被唯一地表示为基的线性组合
$$
\boldsymbol{x}=\alpha_1\boldsymbol{a}_1+\cdots+\alpha_k\boldsymbol{a}_k
$$
其中向量 $\boldsymbol{x}$ 的坐标为
$$
\boldsymbol{x}=
\begin{bmatrix}
\alpha_1\\
...
设 $V$ 和 $W$ 为两向量空间, 若映射 $\varphi$ 对于所有向量 $\boldsymbol{x},\boldsymbol{y}\in V$ 满足线性性
$$
\varphi(k\boldsymbol{x}+l\boldsymbol{y})=k\varphi(\boldsymbol{x})+l\varphi(\boldsymbol{y})
$$

则称 $\varphi$ 为从线性空间 $V$ 到 $W$ 的线性映射.
...
$$
\begin{align}
\mathrm{null}(\boldsymbol{A})\oplus\mathrm{range}(\boldsymbol{A}^{\mathrm{T}})
   &= \mathbb{R}^n\\
   \mathrm{null}(\boldsymbol{A}^{\mathrm{T}})\oplus\mathrm{range}(\boldsymbol{A})
   &= \mathbb{R}^m
\end{align}
$$
...
设 $V$ 为线性空间, $\boldsymbol{x}_0\in V$, 且 $U$ 是 $V$ 的子空间, 则 $V$ 的仿射子空间或称线性流形为
$$
L=U+\boldsymbol{x}_0=\{\boldsymbol{x}_0+\boldsymbol{u}:\boldsymbol{u}\in U\}
$$

设 $U$ 的基为 $\boldsymbol{\alpha}_1, \cdots, \boldsymbol{\alpha}_m$, 则任意向量 $\boldsymbol{x}\in L$ 可以唯一地表示为
$$
\boldsymbol{x}=\boldsymbol{x}_0+a_1\boldsymbol{\alpha}_1+\cdots+ a_m\boldsymbol{\alpha}_m
$$

一维仿射子空间称为直线, 其参数方程为
$$
\boldsymbol{y}=\boldsymbol{x}_0+\lambda_1\boldsymbol{x}_1
$$

二维仿射子空间称为平面, 其参数方程为
$$
\boldsymbol{y}=\boldsymbol{x}_0+\lambda_1\boldsymbol{x}_1+\lambda_2\boldsymbol{x}_2
$$

对于线性非齐次方程组 $\boldsymbol{A}\boldsymbol{x}=\boldsymbol{b}$, 其解空间为 $n-r$ 维仿射子空间.

### 仿射映射

设 $V$ 和 $W$ 为两向量空间, $\Phi:V\rightarrow W$ 为线性映射, 向量 $\boldsymbol{a}\in W$. 我们称 $\varphi$ 为从 $V$ 到 $W$ 的仿射映射, $\boldsymbol{a}$ 为 $\varphi$ 的平移向量
$$
\varphi:V\rightarrow W,~\boldsymbol{x}\rightarrow \boldsymbol{a}+\Phi(\boldsymbol{x})
$$